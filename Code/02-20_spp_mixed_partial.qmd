...
glimpse(spp_rcbd_df)
```
If anova, treatments must be factors, because we have to find the mean response variable. In the contrary, if we are to do a regression, we leave the treatments as numbers. 
...
krate in both fixed effect side of the model and random effect side. There are two errors here, one from the random and one from the fixed (whatever error is left from the random effect, is added to the residual error.)
